...
try {
  ...
  const images = [];
  for (const mode of ['overlay', 'cutout', 'white', 'black', 'original']) {
    await page.locator(`[data-preview="${mode}"]`).click();
    images.push(await page.locator('#editCanvas').evaluate(c => c.toDataURL()));
  }
  assert.equal(new Set(images).size, 5, 'all five previews render distinct views');
  ...
  await page.click('#btnRedoMask'); assert.deepEqual(await signature(page), initial);
  await page.evaluate(() => {
    Segmenter.tapSelect = async (canvas, points) => {
      await new Promise(r => setTimeout(r, 150));
      const w = canvas.width, h = canvas.height, p = points[0];
      return Float32Array.from({ length: w * h }, (_, i) => Math.hypot(i % w - p.x * w, Math.floor(i / w) - p.y * h) < 18 ? 1 : 0);
    };
  });
  await page.locator('[data-tool="tapRemove"]').click();
  const tapPoint = await centre(page); await page.mouse.click(tapPoint.x, tapPoint.y);
  ...
  await page.waitForFunction(() => document.querySelector('#editBusy').hidden);
  assert((await signature(page)).sum < initial.sum, 'tap remove still applies a model region');
  await page.click('#btnUndo'); assert.deepEqual(await signature(page), initial);
  ...
  console.log('PASS async tap busy state, colour key, stroke cancellation and zoom limits');
